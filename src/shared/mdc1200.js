...
export function buildMdcFrame(op, arg, unitId) {
    ...
    for (let source = 0; source < 112; source += 1)
        tx[INTERLEAVE[source]] = src[source];
...
export function encodeMdcBurst(unitId, op = MDC_OP_PTT_ID, arg = MDC_ARG_PTT_ID, sampleRate = 8000, amplitude = 0.6) {
 ...
 */
function discriminator(samples, sampleRate) {
...
function demodulate(disc, sampleRate, offset, samplesPerBit) {
    const bits = [];
    let lastBit = 0;
...
function syncErrors(bits, at, target) {
    let errors = 0;
...
function decodeFrame(bits, at) {
    if (at + 112 > bits.length)
        return null;
    const tx = bits.slice(at, at + 112);
    const src = new Array(112).fill(0);
    for (let source = 0; source < 112; source += 1)
        src[source] = tx[INTERLEAVE[source]];
    const data = new Array(14).fill(0);
    for (let i = 0; i < 14; i += 1)
        for (let j = 0; j <= 7; j += 1)
            data[i] |= src[i * 8 + j] << j;
 ...
 */
export function decodeMdcBursts(input, sampleRate = 8000) {
    const samples = input instanceof Float32Array
        ? input
